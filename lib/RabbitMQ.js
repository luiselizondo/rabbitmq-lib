...
// Follows the RPC pattern described at
// https://www.rabbitmq.com/tutorials/tutorial-six-python.html
RabbitMQ.prototype.consumeRequests = function(requestsToConsume) {
  return this._createChannel()
  .then((channel) => {
    requestsToConsume.forEach((taskName) => {
      channel.assertQueue(taskName, {
        durable: true
      })
      ...
      this._consumeMessagesFromRPCServer(taskName, channel)
    })
  })
}


RabbitMQ.prototype._consumeMessagesFromRPCServer = function(queue, channel) {
  channel.consume(queue, (message) => {
...
// Follows the RPC pattern described at
// https://www.rabbitmq.com/tutorials/tutorial-six-python.html
RabbitMQ.prototype.answerRequest = function(channel, message, callback) {
  var response = {
    type: null
  };
