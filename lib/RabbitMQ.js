...
RabbitMQ.prototype.publishToTopic = function (eventName, data) {
  this._createChannel()
  .then((channel) => {
		if (channel) {
			var data = message || {};
			data.tid = uuid();
			channel.assertExchange(this.EXCHANGE_NAME, 'topic', {durable: false});
			return channel.publish(this.EXCHANGE_NAME, topic, new Buffer(JSON.stringify(data)))
		}
		else {
...
RabbitMQ.prototype.dispatchToQueue = function (queueName, data) {
  console.log('Called dispatchToQueue')
  ...
  .then((channel) => {
    ...
    .then((q) => {
      return channel.sendToQueue(queueName, new Buffer(message), {persistent: true});
    })
  })
  .catch((error) => {
    return Q.fcall(function () {
...
RabbitMQ.prototype.consumeFromQueue = function (tasksToConsume) {
  return this._createChannel()
  .then((channel) => {
    eventsToConsume.forEach((eventName) => {
			channel.assertQueue(eventName, {durable: true})
			channel.prefetch(1);
			this._consumeMessagesOnQueue(eventName, channel)
    })
  })
}

RabbitMQ.prototype._consumeMessagesOnQueue = function (queue, channel) {
  channel.consume(eventName, function (message) {
		try {
